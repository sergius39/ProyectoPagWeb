...
$sql_pedido = "SELECT idpedido, fechaPedido, totalPedido FROM pedido WHERE idcliente = ? ORDER BY fechaPedido DESC LIMIT 1";
  ...
  if ($result_pedido->num_rows > 0) {
    ...
    if ($stmt_detalle_pedido = $conn->prepare($sql_detalle_pedido)) {
      ...
      if ($result_detalle_pedido->num_rows > 0) {
        ...
        echo'<div class="table-responsive table-ultimo-pedido">';
        echo '<table class="table">';
          echo '<thead class="thead-dark">
                <tr>
                  <th style="background-color: #008000; color: white;">Producto</th>
        ...
        while ($detalle_pedido = $result_detalle_pedido->fetch_assoc()) {
          echo "<tr>";
          echo "<td class='letra-cantidad-start'>" . ucfirst($detalle_pedido['nombre_producto']) . "</td>";
          echo "<td class='letra-cantidad-centro'>" . $detalle_pedido['cantidad'] . "</td>";
          echo "</tr>";
        }
        echo '</tbody>';
        echo '<tfoot>';
        echo '<tr><td class="letra-cantidad-start"><strong>Fecha</strong></td><td>' . date("Y-m-d \a\ \l\a\s H:i:s", strtotime($pedido['fechaPedido'])) . '</td></tr>';
        echo '<tr><td class="letra-cantidad-start"><strong>Total Pedido</strong></td><td>' . $pedido['totalPedido'] . '</td></tr>';
        echo '</tfoot>';
        echo '</table>';
        echo '</div>';
      } else {
